      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/" className="flex items-center gap-2 group transition-transform active:scale-95">
            <Image src={logo} alt="Furniro Logo" width={40} height={40} className="w-8  h-auto object-contain" />
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          {!isAuthPage && (
            <>
              ...
              {/* Orders Icon - Show on Desktop */}
              {user && (
                <Link href="/orders" className="hidden sm:inline-block">
                  <button className="text-xs font-medium text-gray-600 cursor-pointer hover:text-amber-700 transition-colors py-1.5 px-3 rounded-md hover:bg-amber-50/50 border border-gray-300">
                    My Orders
                  </button>
                </Link>
              )}
        ...
        <div className="md:hidden absolute top-full left-0 w-full bg-white border-b border-gray-100 px-6 py-4 flex flex-col gap-3 shadow-md animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          {user && (
            <Link
              href="/orders"
              onClick={() => setIsMenuOpen(false)}
              className="text-sm font-medium text-gray-600 hover:text-gray-900 px-3 py-2.5 rounded-lg hover:bg-gray-50 border-t border-gray-100 mt-1"
            >
              My Orders
            </Link>
          )}
